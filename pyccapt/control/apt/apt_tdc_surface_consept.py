...
class APT_ADVANCE:
    ...
    def __init__(self, queue_x, queue_y, queue_t, queue_dld_start_counter,
        ...
        self.conf = conf
        if variables.log:
            self.log_apt_tdc_surface_consept = loggi.logger_creator('apt_tdc_surface_consept',
                                                                'apt_tdc_surface_consept.log', path=variables.log_path)
    ...
    def initialize_v_dc(self):
        ...
        """
        if variables.log:
            self.log_apt_tdc_surface_consept.info("Function - initialize_v_dc | Port selection -> {}".format(
    ...
    def main_ex_loop(self, counts_target):
        ...
        if variables.log:
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | count_temp | value - {}| type - {}".format(
                variables.count_temp, type(variables.count_temp)))
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | count_last | value - {}| type - {}".format(
                variables.count_last, type(variables.count_last)))
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | main_v_dc | value - {}| type - {}".format(
                variables.main_v_dc, type(variables.main_v_dc)))
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | main_counter | value - {}| type - {}".format(
                variables.main_counter, type(variables.main_counter)))
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | avg_n_count | value - {}| type - {}".format(
                variables.avg_n_count, type(variables.avg_n_count)))
            self.log_apt_tdc_surface_consept.info("Function - main_ex_loop | ex_freq | value - {}| type - {}".format(
                variables.ex_freq, type(variables.ex_freq)))
        ...
        def cleanup_variables():
            ...
            if variables.log:
                self.log_apt_tdc_surface_consept.info("Function - cleanup_variables | ch1 | value - {}| type - {}".format(
                    variables.count_temp, type(variables.count_temp)))
            if variables.log:
                self.log_apt_tdc_surface_consept.info("Function - cleanup_variables | main_v_dc_tdc | value - {}| type - {}".format(
                    variables.main_v_dc_tdc, type(variables.main_v_dc_tdc)))
...
def main(conf):
    ...
    if conf['tdc'] != "off" and variables.counter_source != 'DRS':
        ...
        drs_process = multiprocessing.Process(target=drs.experiment_measure, args=(queue_ch0_time, queue_ch0_wave,
                                                                                   ...
                                                                                   queue_stop_measurement,
                                                                                   variables.log, variables.log_path))
