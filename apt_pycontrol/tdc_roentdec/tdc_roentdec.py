...
class tdc_roentdec(object):
    ...
    def get_data_tdc(self, ):
        ...
        """
        data = self.tdc_lib.get_data_tdc(self.obj)
        return data
...
def experiment_measure(queue_x, queue_y, queue_t, queue_AbsoluteTimeStamp,
    ...
    try:
        # load the library
        os.chdir(os.path.split(MODULE_DIR)[0] + '\\tdc_roentdec\\')
        tdc_lib = ctypes.CDLL("./simple_read_TDC8HP_x64.dll")
    except:
        print("TDC DLL was not found")
    tdc = tdc_roentdec(tdc_lib, buf_size=buf_size, time_out=time_out)
    ...
    while True:
        ...
        buffer_length = returnVale[0]
        returnVale = returnVale[1:].reshape(12, buffer_length)

        queue_x.put(returnVale[0])
        queue_y.put(returnVale[1])
        queue_t.put(returnVale[2])
        queue_AbsoluteTimeStamp.put(returnVale[3])
        queue_ch0.put(returnVale[4])
        queue_ch1.put(returnVale[5])
        queue_ch2.put(returnVale[6])
        queue_ch3.put(returnVale[7])
        queue_ch4.put(returnVale[8])
        queue_ch5.put(returnVale[9])
        queue_ch6.put(returnVale[10])
        queue_ch7.put(returnVale[11])

        if queue_stop_measurement.get():
            break
